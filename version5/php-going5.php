...
class IPUtils {
     ...
     * Handles proper byte order for Linux /proc format
     */
    public static function hexToIpv6(string $hex): string {
        ...
        $groups = [];
        for ($i = 0; $i < 32; $i += 4) {
            // Reverse the byte order within each 16-bit word
            $word = substr($hex, $i, 4);
            $reversed = substr($word, 2, 2) . substr($word, 0, 2);
            $groups[] = dechex(hexdec($reversed));
        }
     ...
     */
    public static function compressIPv6(string $ipv6): string {
