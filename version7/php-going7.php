...
class ProcessCache {
    ...
    private static function buildProcessMap(): array {
        ...
        // Use more efficient directory scanning focusing on relevant inodes
    ...
    private static function extractInodesFromProcNet(): array {
        ...
        return array_flip($inodes); // Use as keys for faster lookup
    }
...
class ConnectionWatcher {
    ...
    private function displayChanges(array $changes, int $iteration): void {
        ...
        $totalChanges = $changes['total_changes'] ?? (count($changes['added']) + count($changes['removed']));
        if ($totalChanges === 0) {
